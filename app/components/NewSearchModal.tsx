...
export default function NewSearchModal({
  ...
  function handleSave() {
    if (isEditing && editSearch && onSave) {
      ...
      alert(`"${name}" is live! You'll get alerts within minutes.`);
    }
    onClose();
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end" style={{ background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(8px)' }}>
      <div
        ref={sheetRef}
        style={{ background: '#1C1C1E', borderRadius: '20px 20px 0 0', width: '100%', maxHeight: '92vh', display: 'flex', flexDirection: 'column', transition: 'transform 0.15s ease', willChange: 'transform' }}
      >
        {/* Drag handle */}
        <div
        ...
        <div className="px-5 pt-2 pb-3" style={{ borderBottom: '0.5px solid rgba(255,255,255,0.1)' }}>
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={onClose} style={{ width: 30, height: 30, background: '#3A3A3C', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
