export default function Logo({ size = 'md' }: { size?: 'sm' | 'md' | 'lg' }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
      <div style={{ width: s.box, height: s.box, borderRadius: 10, background: '#FB471F', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxShadow: '0 2px 8px rgba(251,71,31,0.35)' }}>
      ...
      <span style={{ fontWeight: 800, fontSize: s.fontSize, letterSpacing: -0.6, color: '#1A1A2E', lineHeight: 1 }}>
        Flip<span style={{ color: '#FB471F' }}>Alert</span>
      </span>
    </div>
  );
}
