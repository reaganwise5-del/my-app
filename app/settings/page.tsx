  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(8px)', zIndex: 100, display: 'flex', alignItems: 'flex-end' }}>
      <div style={{ background: '#1C1C1E', borderRadius: '20px 20px 0 0', width: '100%', padding: '24px 20px 40px', textAlign: 'center' }}>
        ...
        <button type="button" onClick={onClose} style={{ width: '100%', background: '#2C2C2E', color: '#fff', fontWeight: 600, fontSize: 17, padding: '14px 0', borderRadius: 14 }}>
          Got it
        </button>
      </div>
    </div>
...
function ConnectDataSheet({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(8px)', zIndex: 100, display: 'flex', alignItems: 'flex-end' }}>
      <div style={{ background: '#1C1C1E', borderRadius: '20px 20px 0 0', width: '100%', padding: '0 0 44px', maxHeight: '88vh', overflowY: 'auto' }}>
        ...
        <div style={{ padding: '8px 20px 4px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ color: '#fff', fontWeight: 800, fontSize: 20 }}>Connect Live Data</h2>
        ...
        <p style={{ color: '#8E8E93', fontSize: 13, padding: '0 20px 20px' }}>Pull real Facebook Marketplace listings in 3 steps.</p>
        ...
        <div style={{ padding: '0 20px' }}>
          ...
          <StepCard num={1} active={step === 1} done={step > 1}
            ...
            action={<a href="https://apify.com/sign-up" target="_blank" rel="noreferrer"
              style={{ display: 'inline-block', background: '#22c55e', color: '#000', fontWeight: 700, fontSize: 14, padding: '10px 20px', borderRadius: 12 }}>
              Sign Up Free →
            </a>}
          ...
          <StepCard num={2} active={step === 2} done={step > 2}
            ...
            desc={<>After signing up, go to <strong style={{ color: '#fff' }}>apify.com/account → Integrations</strong> and copy your Personal API token.</>}
            action={
              <div style={{ background: '#2C2C2E', borderRadius: 12, padding: 12 }}>
                <p style={{ color: '#636366', fontSize: 11, marginBottom: 4 }}>It looks like this:</p>
                <code style={{ color: '#22c55e', fontSize: 13 }}>apify_api_xxxxxxxxxxxxxxxxxxxxxxxxxxxx</code>
              </div>
            }
          ...
          <StepCard num={3} active={step === 3} done={false}
            ...
            desc={<>Open the file <strong style={{ color: '#fff' }}>.env.local</strong> in your FlipAlert project folder and paste your key next to <code style={{ color: '#22c55e' }}>APIFY_API_KEY=</code></>}
            action={
              <div>
                <div style={{ background: '#000', borderRadius: 12, padding: 12, fontFamily: 'monospace' }}>
                  <p style={{ color: '#636366', fontSize: 12 }}># .env.local</p>
                  <p style={{ color: '#22c55e', fontSize: 13 }}>APIFY_API_KEY=apify_api_your_key_here</p>
                </div>
                <p style={{ color: '#636366', fontSize: 12, marginTop: 8 }}>Then restart the dev server (<code style={{ color: '#8E8E93' }}>npm run dev</code>) and refresh the app. The feed will automatically switch to live listings.</p>
              </div>
            }
            onContinue={onClose}
            continueLabel="Done ✓"
...
function StepCard({ num, active, done, title, desc, action, onContinue, continueLabel = 'Continue' }: {
  ...
  return (
    <div style={{ background: active ? '#2C2C2E' : done ? 'rgba(34,197,94,0.05)' : '#2C2C2E', borderRadius: 16, padding: 16, marginBottom: 12, opacity: (!active && !done) ? 0.5 : 1, transition: 'all 0.2s' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
        <div style={{ width: 28, height: 28, borderRadius: '50%', background: done ? '#22c55e' : active ? '#fff' : '#3A3A3C', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          {done
            ? <svg width="14" height="14" fill="none" viewBox="0 0 24 24"><path d="M5 13l4 4L19 7" stroke="#000" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
            : <span style={{ color: done || active ? '#000' : '#8E8E93', fontSize: 13, fontWeight: 700 }}>{num}</span>
          }
        </div>
        <p style={{ color: done ? '#22c55e' : '#fff', fontWeight: 700, fontSize: 15 }}>{title}</p>
      </div>
      {active && (
        <>
          <p style={{ color: '#8E8E93', fontSize: 13, marginBottom: 14, lineHeight: 1.5 }}>{desc}</p>
          <div style={{ marginBottom: 14 }}>{action}</div>
          <button type="button" onClick={onContinue}
            style={{ width: '100%', background: '#22c55e', color: '#000', fontWeight: 700, fontSize: 15, padding: '12px 0', borderRadius: 12 }}>
            {continueLabel}
          </button>
        </>
      )}
    ...
    <div className="min-h-screen pb-safe" style={{ background: '#000' }}>
      ...
      <Section title="Data Source">
        <div onClick={() => setShowConnectData(true)} role="button"
          style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 0', borderBottom: '0.5px solid rgba(255,255,255,0.06)', cursor: 'pointer' }}
          className="active:opacity-60 transition-opacity">
          <div>
            <p style={{ color: '#fff', fontSize: 15 }}>Facebook Marketplace</p>
            <p style={{ color: apifyConnected ? '#22c55e' : '#eab308', fontSize: 12, marginTop: 2 }}>
